...
export function server(ns, host, ram) {
	return {
		...
		infect: function () { if (this.hostname.includes(this.owned_prefix)) this.infected = true; if (!this.infected && this.get_opened_ports() <= this.get_required_ports()) { if (this.exploits.length >= this.get_required_ports()) { if (ns.getHackingLevel() >= this.get_required_level()) { if (this.exploits.length >= 1) ns.brutessh(this.hostname); if (this.exploits.length >= 2) ns.ftpcrack(this.hostname); if (this.exploits.length >= 3) ns.relaysmtp(this.hostname); if (this.exploits.length >= 4) ns.httpworm(this.hostname); if (this.exploits.length >= 5) ns.sqlinject(this.hostname); } } } if (this.get_opened_ports() >= this.get_required_ports() && !this.get_root()) { ns.nuke(this.hostname); this.infected = true; } if (this.get_root()) this.infected = true; if (this.infected) { if (this.get_thread_count() > 0) this.run_scripts(); return 1; } else { return 0; } },
...
export async function infect_all(ns, hosts) {
	for (let victim of hosts) { if (!victim.infected) victim.infect(); } await ns.sleep(1);
}

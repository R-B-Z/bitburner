...
export function server(ns, host, ram) {
	return {
		...
		run_scripts: function () { this.fetch_dependencies(); for (let script of this.scripts) { if (!ns.isRunning(script, this.hostname) && this.get_thread_count() > 0) { ns.scp(script, this.hostname); if (script == 'payload.js') { ns.exec(script, this.hostname, this.get_thread_count()); } else if (this.hostname == 'home') { ns.exec(script, this.hostname, 1); } } } },
